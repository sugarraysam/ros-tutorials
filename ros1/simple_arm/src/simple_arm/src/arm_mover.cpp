...
bool handle_safe_move_request(simple_arm::GoToPosition::Request &req,
  ...
  ROS_INFO("GoToPositionRequest received - j1:%1.2f, j2:%1.2f",
           (float)req.joint_1, (float)req.joint_2);

  std::vector<float> joints_angles =
      clamp_at_boundaries(req.joint_1, req.joint_2);
  ...
  res.msg_feedback =
      "Joint angles set - j1: " + std::to_string(joints_angles[0]) +
...
int main(int argc, char **argv) {
  ...
  joint1_pub = n.advertise<std_msgs::Float64>(
      "/simple_arm/joint_1_position_controller/command", 10);
  joint2_pub = n.advertise<std_msgs::Float64>(
      "/simple_arm/joint_2_position_controller/command", 10);

  ros::ServiceServer service =
      n.advertiseService("/arm_mover/safe_move", handle_safe_move_request);
